...
import '../assets/css/courses-rtl.css';
            ...
            <div className={`max-w-4xl mx-auto text-white ${isRTL ? 'text-right' : 'text-center'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-3xl mx-auto">
                <div className={`${isRTL ? 'text-center' : 'text-center'}`}>
                  <div className="text-4xl md:text-5xl font-bold text-white mb-2" dir="ltr">{totalCourses}+</div>
                  <div className={`text-white/80 text-lg ${isRTL ? 'font-arabic' : ''}`}>{t('hero.stats.courses')}</div>
                </div>
                <div className={`${isRTL ? 'text-center' : 'text-center'}`}>
                  <div className="text-4xl md:text-5xl font-bold text-white mb-2" dir="ltr">{totalInstructors}+</div>
                  <div className={`text-white/80 text-lg ${isRTL ? 'font-arabic' : ''}`}>{t('hero.stats.instructors')}</div>
                </div>
                <div className={`${isRTL ? 'text-center' : 'text-center'}`}>
                  <div className="text-4xl md:text-5xl font-bold text-white mb-2" dir="ltr">{totalInstitutions}+</div>
                  <div className={`text-white/80 text-lg ${isRTL ? 'font-arabic' : ''}`}>{t('hero.stats.institutions')}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className={`flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
              <div className={`space-y-2 ${isRTL ? 'text-right' : ''}`}>
                ...
                <p className="text-muted-foreground text-lg">
                  <span dir="ltr">{filteredCourses.length}</span> {filteredCourses.length !== 1 ? t('content.coursesFoundPlural') : t('content.coursesFound')} {t('content.found')}
                </p>
              </div>
