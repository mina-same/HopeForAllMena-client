...
export const BookCard = ({ book, viewMode }) => {
  ...
  // Use pre-transformed data from Bookstore component, with fallback for direct usage
  const bookTitle = book.title; // Already transformed in Bookstore component
  const bookDescription = book.description; // Already transformed in Bookstore component
  const authorName = book.authorName || // Use pre-transformed author name
    (currentLanguage === 'ar' ? 
      (book.author?.nameAr || book.author?.name) : book.author?.name); // Fallback for direct usage
    ...
    return (
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-sm text-muted-foreground">
          ({t('bookCard.reviews', { count: book.reviewCount || book.totalReviews || 0 })})
        </span>
      </div>
    );
  };
      ...
      <>
        ...
        {currentLanguage === 'ar' && (
          <style jsx>{`
            ...
            .book-card-list .text-left {
              text-align: right;
            }
            .book-card-list .justify-start {
              justify-content: flex-end;
            }
            .book-card-list .items-start {
              align-items: flex-end;
            }
          `}</style>
        )}
        <div className={`group relative bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-2xl hover:border-[#2194D1]/30 transition-all duration-500 w-full book-card-list ${currentLanguage === 'ar' ? 'rtl' : ''}`} dir={currentLanguage === 'ar' ? 'rtl' : 'ltr'}>
        <div className={`grid grid-cols-12 gap-6 p-6 ${currentLanguage === 'ar' ? 'rtl-grid' : ''}`}>
          ...
          <div className="col-span-12 sm:col-span-3 lg:col-span-2">
            <Link to={`/book/${book.id}`} className="block">
              <div className="relative overflow-hidden rounded-xl group-hover:shadow-lg transition-all duration-300">
                <img
                  src={book.image || book.coverImageUrl}
                  alt={bookTitle}
              ...
              {/* Category Badge */}
              <div className={`absolute top-3 ${currentLanguage === 'ar' ? 'right-3' : 'left-3'}`}>
          ...
          <div className="col-span-12 sm:col-span-6 lg:col-span-7">
            <div className={`h-full flex flex-col justify-between ${currentLanguage === 'ar' ? 'text-right' : 'text-left'}`}>
              {/* Title and Author */}
              <div className="mb-4">
                <Link to={`/book/${book.id}`}>
                  <h3 className={`text-2xl lg:text-3xl font-bold text-gray-900 mb-3 hover:text-[#2194D1] transition-colors cursor-pointer leading-tight ${currentLanguage === 'ar' ? 'text-right' : 'text-left'}`}>
                    {bookTitle}
                  </h3>
                </Link>
                <p className={`text-lg text-gray-600 font-medium ${currentLanguage === 'ar' ? 'text-right' : 'text-left'}`}>
                  {t('bookCard.byAuthor')} <span className="text-gray-800 font-semibold">{authorName || t('bookCard.unknownAuthor')}</span>
                </p>
              </div>

              {/* Rating and Reviews */}
              <div className="mb-4">
                <div className={`flex items-center gap-3 ${currentLanguage === 'ar' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              {bookDescription && (
                <div className="mb-6">
                  <p className={`text-gray-600 leading-relaxed line-clamp-2 lg:line-clamp-3 ${currentLanguage === 'ar' ? 'text-right' : 'text-left'}`}>
                    {bookDescription}
                  </p>
                </div>
              )}
              ...
              {/* Additional Info */}
              <div className={`flex flex-wrap gap-4 mb-4 ${currentLanguage === 'ar' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`flex items-center gap-2 text-sm text-gray-500 ${currentLanguage === 'ar' ? 'flex-row-reverse' : 'flex-row'}`}>
    ...
    <>
      ...
      {currentLanguage === 'ar' && (
        <style jsx>{`
          ...
          .book-card-grid .aspect-ratio {
            direction: ltr; /* Keep image aspect ratio LTR */
          }
        `}</style>
      )}
      <div className={`group relative w-full max-w-xs xs:max-w-sm mx-auto bg-card rounded-xl xs:rounded-2xl shadow-md overflow-hidden transition-all duration-500 hover:shadow-2xl hover:scale-[1.02] border border-border/50 book-card-grid ${currentLanguage === 'ar' ? 'rtl' : ''}`} dir={currentLanguage === 'ar' ? 'rtl' : 'ltr'}>
      {/* Book Cover Section */}
      <div
      ...
      <div className="p-3 xs:p-4">
        ...
        <Link to={`/book/${book.id}`}>
          <h3 className="text-sm xs:text-base sm:text-lg font-bold text-foreground mb-2 xs:mb-3 hover:text-[#2194D1] cursor-pointer transition-colors duration-200 line-clamp-2 leading-tight">
            {bookTitle}
          </h3>
        </Link>

        {/* Author with enhanced styling */}
        <p className="text-xs xs:text-sm text-muted-foreground mb-2 xs:mb-3 font-medium">
          {t('bookCard.byAuthor')} {authorName || t('bookCard.unknownAuthor')}
        </p>
        ...
        <div className="flex items-center gap-1 xs:gap-2 mb-3 xs:mb-4">
          <div className="flex items-center">
            {[1, 2, 3, 4, 5].map((star) => (
              <svg
                key={star}
                className={`w-3 xs:w-4 h-3 xs:h-4 transition-colors ${star <= Math.floor(book.rating) ? 'text-yellow-400 fill-current' :
